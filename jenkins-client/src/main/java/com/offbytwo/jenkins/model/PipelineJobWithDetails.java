...
public class PipelineJobWithDetails extends PipelineJob {
	...
	private List<PipelineBuild> builds;

	private PipelineBuild firstBuild;

	private PipelineBuild lastBuild;

	private PipelineBuild lastCompletedBuild;

	private PipelineBuild lastFailedBuild;

	private PipelineBuild lastStableBuild;

	private PipelineBuild lastSuccessfulBuild;

	private PipelineBuild lastUnstableBuild;

	private PipelineBuild lastUnsuccessfulBuild;
	 ...
	 * {@link #getAllBuilds()}.
	 * 
	 * @return the list of {@link PipelineBuild}. In case of no builds have been
	 *         executed yet return {@link Collections#emptyList()}.
	 */
	public List<PipelineBuild> getBuilds() {
		if (builds == null) {
			return Collections.emptyList();
		} else {
			return transform(builds, new Function<PipelineBuild, PipelineBuild>() {
				@Override
				public PipelineBuild apply(PipelineBuild from) {
	 ...
	 * particular build {@link PipelineBuild#details()} to reduce the amount of
	 * data which needed to be transfered.
	 * 
	 * @return the list of {@link PipelineBuild}. In case of no builds have been
	 *         executed yet return {@link Collections#emptyList()}.
	 * @throws IOException
	 ...
	 *      Issue</a>
	 */
	public List<PipelineBuild> getAllBuilds() throws IOException {
		String path = "/";

		try {
			List<PipelineBuild> builds = client.get(path + "job/" + EncodingUtils.encode(this.getName())
					+ "?tree=allBuilds[number[*],url[*],queueId[*]]", AllPipelineBuilds.class).getAllBuilds();
	 ...
	 *             in case of an error.
	 */
	public List<PipelineBuild> getAllBuilds(Range range) throws IOException {
		String path = "/" + "job/" + EncodingUtils.encode(this.getName())
				+ "?tree=allBuilds[number[*],url[*],queueId[*]]";

		try {
			List<PipelineBuild> builds = client.get(path + range.getRangeString(), AllPipelineBuilds.class)
					.getAllBuilds();
	...
	private PipelineBuild buildWithClient(PipelineBuild from) {
		PipelineBuild ret = from;
		if (from != null) {
			ret = new PipelineBuild(from);
	 ...
	 *         {@link PipelineBuild#BUILD_HAS_NEVER_RUN} if the build has never
	 *         been run.
	 */
	public PipelineBuild getFirstBuild() {
		if (firstBuild == null) {
			return PipelineBuild.BUILD_HAS_NEVER_RUN;
		} else {
	 ...
	 *         {@link PipelineBuild#BUILD_HAS_NEVER_RUN} will be returned.
	 */
	public PipelineBuild getLastBuild() {
		if (lastBuild == null) {
			return PipelineBuild.BUILD_HAS_NEVER_RUN;
		} else {
	 ...
	 *         been run {@link PipelineBuild#BUILD_HAS_NEVER_RUN} will be
	 *         returned.
	 */
	public PipelineBuild getLastCompletedBuild() {
	 ...
	 *         run {@link PipelineBuild#BUILD_HAS_NEVER_RUN} will be returned.
	 */
	public PipelineBuild getLastFailedBuild() {
	 ...
	 *         run {@link PipelineBuild#BUILD_HAS_NEVER_RUN} will be returned.
	 */
	public PipelineBuild getLastStableBuild() {
	 ...
	 *         never been run {@link PipelineBuild#BUILD_HAS_NEVER_RUN} will be
	 *         returned.
	 */
	public PipelineBuild getLastSuccessfulBuild() {
	 ...
	 *         returned.
	 */
	public PipelineBuild getLastUnstableBuild() {
	 ...
	 *         returned.
	 */
	public PipelineBuild getLastUnsuccessfulBuild() {
	 ...
	 * @return The {@link PipelineBuild} selected by the given buildnumber
	 * 
	 */
	public PipelineBuild getBuildByNumber(final int buildNumber) {

		Predicate<PipelineBuild> isMatchingBuildNumber = new Predicate<PipelineBuild>() {

			@Override
			public boolean apply(PipelineBuild input) {
				return input.getNumber() == buildNumber;
			}
		};

		Optional<PipelineBuild> optionalBuild = Iterables.tryFind(builds, isMatchingBuildNumber);
		// TODO: Check if we could use PipelineBuild#NO...instead of Null?
	 ...
	 * 
	 * @param description
	 *            The description which should be set. If you like to set an
	 *            empty description you should use {@link #EMPTY_DESCRIPTION}.
	 * @throws IOException
	 *             in case of errors.
	 */
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PipelineJobWithDetails other = (PipelineJobWithDetails) obj;
